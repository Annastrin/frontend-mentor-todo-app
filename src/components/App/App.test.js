...
it('should change task checkbox state on click', () => {
  ...
  const checkbox = screen.getByRole('checkbox', {name: 'Complete online JavaScript course'});
...
it('should add a new task', () => {
  ...
  const addedTask = screen.getByRole('checkbox', {name: 'Add a test for AddTask'});
  expect(addedTask).toBeInTheDocument();
});

it('should remove a task', () => {
  customRender(<App/>, {initialState: {...appInitialState}});
  const completedTask = screen.getByRole('checkbox', {name: 'Add linter to the project'});
  const removeBtn = screen.getByTestId(name2KebabWithId('Add linter to the project', 1));
...
it('should show active tasks', () => {
  customRender(<App/>, {initialState: {...appInitialState}});
  const activeTask = screen.getByRole('checkbox', {name: 'Complete online JavaScript course'});
  const completedTask = screen.getByRole('checkbox', {name: 'Add linter to the project'});
